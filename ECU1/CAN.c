 ...
 * Author: HP
 *
 * Created on May 13, 2025, 9:21 PM
 */
...
unsigned char can_receive(unsigned int *msg_id,unsigned int *rec_data,unsigned int *len)
{
	...
	if (RXB0FUL) /* CheckRXB0 */ // IF ITS 1 IT MEANS DATA IS TSILL GETTING ERCEIIEVED 
	{
//        clcd_putch('P',LINE2(11));
		can_payload[EIDH] = RXB0EIDH;
		can_payload[EIDL] = RXB0EIDL;
		*msg_id = RXB0SIDH;
		//can_payload = RXB0SIDL;
		*len = RXB0DLC;
		rec_data[0] = RXB0D0;
       // clcd_putch(RXB0D0 ,LINE2(4));
		rec_data[1] = RXB0D1;
         //clcd_putch(RXB0D1 ,LINE2(5));
		rec_data[2] = RXB0D2;
		rec_data[3] = RXB0D3;
		rec_data[4] = RXB0D4;
		rec_data[5] = RXB0D5;
...
unsigned short can_transmit(unsigned char ID,unsigned int *data,unsigned int len) // transmit(uint16_t msg)
{
	TXB0EIDH = 0x00;		/* Extended Identifier */
	TXB0EIDL = 0x00;		/* Extended Identifier */

	/* 0x35E  0110 1011 110 */
	TXB0SIDH = ID;		/* Standard Identifier */   //msg_id
...
//    clcd_putch(data[1],LINE2(9));
	...
	TXB0REQ = 1;			/* Set the buffer to transmit */ // REQUESTING FOR TRANSMISSION
    while(TXB0REQ); // WAIT UNTILL TRANSMISSION IS DONE, AUTOMATICALLY BECOMES ZERO
